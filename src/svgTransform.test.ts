...
describe('convertJsxToSvg', () => {
  it('should convert expression values {number} to quoted strings (Base64 encoded)', () => {
    ...
    const expected =
      '<svg><path data-better-svg-temp-stroke-width="__JSX_BASE64__Mg==__" /></svg>'
  ...
  it('should handle complex JSX SVG', () => {
    ...
    assert.ok(
      result.includes('stroke-linecap='),
      'should convert strokeLinecap'
    )
    assert.ok(
      result.includes('stroke-linejoin='),
      'should convert strokeLinejoin'
    )
    assert.ok(
      result.includes('stroke-linejoin='),
      'should convert strokeLinejoin'
    ...
    assert.ok(
      result.includes(
        'data-better-svg-temp-stroke-width="__JSX_BASE64__Mg==__"'
      ),
      'should convert strokeWidth={2}'
    )
...
describe('convertSvgToJsx', () => {
  ...
  it('should handle optimized SVG output', () => {
    const input =
      '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>'

    const result = convertSvgToJsx(input)

    assert.ok(
      result.includes('className='),
      'should convert class to className'
    )
    assert.ok(
      result.includes('strokeLinecap='),
      'should convert stroke-linecap'
    )
    assert.ok(
      result.includes('strokeLinejoin='),
      'should convert stroke-linejoin'
    )
...
describe('roundtrip conversion', () => {
  it('should preserve attributes after JSX -> SVG -> JSX roundtrip', () => {
    const jsxInput =
      '<svg className="icon"><path strokeWidth="2" strokeLinecap="round" /></svg>'
  ...
  it('should handle all stroke attributes in roundtrip', () => {
    const jsxInput =
      '<svg><path strokeWidth="2" strokeLinecap="round" strokeLinejoin="bevel" strokeDasharray="5,5" strokeOpacity="0.5" /></svg>'
  ...
  it('should handle font attributes in roundtrip', () => {
    const jsxInput =
      '<svg><text fontFamily="Arial" fontSize="12" fontWeight="bold" fontStyle="italic" /></svg>'
...
describe('attribute maps', () => {
  it('should have matching entries in both maps', () => {
    ...
    assert.strictEqual(
      jsxKeys.length,
      svgKeys.length,
      'Maps should have same number of entries'
    )

    for (const [jsx, svg] of Object.entries(jsxToSvgAttributeMap)) {
      assert.strictEqual(
        svgToJsxAttributeMap[svg],
        jsx,
        `${svg} should map back to ${jsx}`
      )
    }
  })
  ...
  it('should include all common stroke attributes', () => {
    ...
    for (const attr of strokeAttrs) {
      assert.ok(
        attr in jsxToSvgAttributeMap,
        `${attr} should be in jsxToSvgAttributeMap`
      )
    }
  })
...
describe('edge cases', () => {
  ...
  it('should handle self-closing SVG elements', () => {
    const input =
      '<svg><circle strokeWidth={2} /><rect strokeWidth={3} /></svg>'
    const result = convertJsxToSvg(input)
    assert.ok(
      result.includes(
        'data-better-svg-temp-stroke-width="__JSX_BASE64__Mg==__"'
    ...
    assert.ok(
      result.includes(
        'data-better-svg-temp-stroke-width="__JSX_BASE64__Mw==__"'
      )
    )
  })
  ...
  it('should handle expression with variable name', () => {
    ...
    assert.ok(
      result.includes(
        'data-better-svg-temp-stroke-width="__JSX_BASE64__c3Ryb2tlU2l6ZQ==__"'
      )
    )
  })
  ...
  it('should handle nested SVG elements', () => {
    ...
    assert.strictEqual(
      classCount,
      2,
      'Should convert both className occurrences'
    )
  })
  ...
  it('should not convert partial attribute names', () => {
    ...
    assert.ok(
      result.includes('data-strokeWidth=') ||
        result.includes('data-stroke-width=')
    )
  })
  ...
  it('should handle mixed quote styles', () => {
    const input =
      '<svg className="icon"><path strokeLinecap=\'round\' strokeWidth={2} /></svg>'
    const result = convertJsxToSvg(input)
    assert.ok(result.includes('class="icon"'))
    assert.ok(result.includes("stroke-linecap='round'"))
  ...
  it('should handle nested brackets in expressions (e.g. onClick handlers)', () => {
    const input =
      '<svg onClick={() => { console.log("click") }} strokeWidth={2}></svg>'
    const result = convertJsxToSvg(input)
...
describe('spread attributes', () => {
  ...
  it('should convert spread attributes in convertJsxToSvg', () => {
    ...
    const expected =
      '<svg data-better-svg-temp-data-spread-0="__JSX_BASE64__cHJvcHM=__"><path /></svg>'
    assert.strictEqual(convertJsxToSvg(input), expected)
  })
  ...
  it('should handle multiple spread attributes', () => {
    ...
    assert.ok(
      svg.includes(
        'data-better-svg-temp-data-spread-0="__JSX_BASE64__cHJvcHM=__"'
      )
    )
    assert.ok(
      svg.includes(
        'data-better-svg-temp-data-spread-1="__JSX_BASE64__dXNlcg==__"'
      )
    )
...
describe('Regressions & Edge Cases', () => {
  it('should preserve className={className} (Base64 encoded)', () => {
    ...
    assert.ok(
      finalResult.includes('className={className}'),
      `Expected className={className}, got: ${finalResult}`
    )
    assert.ok(
      !finalResult.includes('className="className"'),
      'Should not contain string literal className="className"'
    )
  })
  ...
  it('should preserve style object style={{ color: "red" }}', () => {
    ...
    assert.ok(
      preparedSvg.includes('data-better-svg-temp-style='),
      'Style should be renamed'
    )
    assert.ok(
      !preparedSvg.includes(' style='),
      'Original style attribute should be gone'
    )
  ...
  it('should handle the specific Icon2 component causing issues', () => {
    ...
    assert.ok(
      finalResult.includes('strokeWidth="2"') ||
        finalResult.includes("strokeWidth='2'")
    )
  })
  ...
  it('should preserve namespaced/dotted components like <motion.path /> if SVGO allows them', () => {
    ...
    assert.ok(
      finalResult.includes('<motion.path'),
      `motion.path lost: ${finalResult}`
    )
    assert.ok(
      finalResult.includes('animate={{ x: 100 }}'),
      `animate prop lost: ${finalResult}`
    )
  })
})
...
describe('Comments Handling', () => {
  it('should handle JSX block comments {/* ... */}', () => {
    ...
    assert.ok(
      ...
      'Block comment start should be gone or converted'
    )
  ...
  it('should handle JSX line comments // ... inside tag attributes', () => {
    ...
    assert.ok(
      !preparedSvg.includes('// This is a line comment'),
...
describe('Complex Components', () => {
  it('should handle the ComplexIcon properly (stroke={color})', () => {
    ...
    // Check stroke={color}
    assert.ok(
      finalResult.includes('stroke={color}'),
      `Expected stroke={color}, got: ${finalResult}`
    )
    assert.ok(
